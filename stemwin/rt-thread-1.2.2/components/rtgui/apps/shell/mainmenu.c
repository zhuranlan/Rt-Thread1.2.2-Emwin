...
//                                         rtgui_rect_height(view->view_rect));
		    ...
		    view->view_buffer =
        rtgui_dc_buffer_create_pixformat(RTGRAPHIC_PIXEL_FORMAT_RGB565P,
                                         rtgui_rect_width(view->view_rect) *
                                         view->page_count,
                                         rtgui_rect_height(view->view_rect));
...
//			rt_kprintf("move here!\n");
   ...
   // RTGUI_DC_BC(view->view_buffer) = RTGUI_ARGB(0, 0, 0, 0);
    RTGUI_DC_BC(view->view_buffer) = RTGUI_RGB(0, 0, 0);
...
//    RTGUI_DC_BC(view->pm_buffer) = RTGUI_ARGB(0, 0, 0, 0);
    ...
    RTGUI_DC_BC(view->pm_buffer) = RTGUI_RGB(0, 0, 0);
...
static char image_path[128]="/SD/SD.BMP";
static void app_mainmenu_init(void)
{
    ...
    if (win != RT_NULL)
    {
        ...
        ycircle_image = rtgui_image_create("/SD/RESOURCE/ycircle.png", RT_TRUE);
        gcircle_image = rtgui_image_create("/SD/RESOURCE/gcircle.png", RT_TRUE);
        app_list = app_list_create(&rect, items, ITEM_MAX, 2,2, bg_image);
