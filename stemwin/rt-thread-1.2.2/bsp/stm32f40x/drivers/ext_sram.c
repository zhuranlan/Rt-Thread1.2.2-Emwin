...
int ext_sram_init(void)
{
    ...
    Timing_read.FSMC_AddressSetupTime = 3;  /* [3:0] F2/F4 1~15 HCLK    3*/
    Timing_read.FSMC_AddressHoldTime = 0;   /* [7:4] keep 0x00 in SRAM mode */
    Timing_read.FSMC_DataSetupTime = 3;     /* [15:8] F2/F4 0~255 HCLK   8*/
    ...
    Timing_write.FSMC_DataSetupTime = 0;     /* [15:8] F2/F4 0~255 HCLK   8*/
    /* [19:16] Time between NEx high to NEx low (BUSTURN HCLK) */
    ...
    Timing_write.FSMC_AccessMode = FSMC_AccessMode_A;   /* FSMC ∑√Œ ƒ£ Ω */
...
void ext_sram_test(void)
{
    /* memtest */
    {
        ...
        p_extram = (unsigned char *)STM32_EXT_SRAM_BEGIN;
				rt_kprintf("sram:%d \n",(STM32_EXT_SRAM_END-STM32_EXT_SRAM_BEGIN));
        for(temp=0; temp<(STM32_EXT_SRAM_END-STM32_EXT_SRAM_BEGIN); temp++)
        {
