...
*/
	...
	/* 定义GPIO端口 */
	#define RCC_SCK 	RCC_AHB1Periph_GPIOI
...
#ifdef HARD_SPI
	#define SPI2_HARD	SPI2
	#define RCC_SPI2		RCC_APB2Periph_SPI2
...
void bsp_InitSPIBus(void)
{
	...
	/* 配置 SCK, MISO 、 MOSI 为复用功能 */
	GPIO_PinAFConfig(GPIOI, GPIO_PinSource1, GPIO_AF_SPI1);
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1 ;
	...
	SPI2_HARD->I2SCFGR &= SPI_Mode_Select;		/* 选择SPI模式，不是I2S模式 */

	/*---------------------------- SPIx CRCPOLY Configuration --------------------*/
	/* Write to SPIx CRCPOLY */
	SPI2_HARD->CRCPR = 7;		/* 一般不用 */


	SPI_Cmd(SPI2_HARD, DISABLE);			/* 先禁止SPI  */

	SPI_Cmd(SPI2_HARD, ENABLE);			/* 使能SPI  */
#endif	
}
...
void bsp_SPI_Init(uint16_t _cr1)
{
	SPI2_HARD->CR1 = ((SPI2_HARD->CR1 & CR1_CLEAR_Mask) | _cr1);
	  
	//SPI_Cmd(SPI_HARD, DISABLE);			/* 先禁止SPI  */	    
    SPI2_HARD->CR1 &= CR1_SPE_Reset;	/* Disable the selected SPI peripheral */

	//SPI_Cmd(SPI_HARD, ENABLE);			/* 使能SPI  */		    
    SPI2_HARD->CR1 |= CR1_SPE_Set;	  /* Enable the selected SPI peripheral */
}
#endif
